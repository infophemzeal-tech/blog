...
const swalConfig = (isDark: boolean) => ({
  ...
  customClass: {
    ...
    confirmButton:
      'font-sans text-xs px-5 py-2.5 rounded-full font-bold bg-stone-900 dark:bg-white text-white dark:text-black border-none outline-none',
  },
})

// FIX 6 — stop words defined once outside component as a Set (no rebuild on every render)
const STOP_WORDS = new Set([
  'this', 'that', 'with', 'from', 'your', 'their', 'about', 'story', 'into', 'some', 'what',
])
...
export default function WritePage() {
  ...
  // FIX 6 — wrapped in useCallback; STOP_WORDS is a stable Set defined outside
  const autoGetTags = useCallback(() => {
    ...
    setTags(prev => Array.from(new Set([...prev, ...top5])))
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-32 font-sans">
      ...
      <div className="flex items-center justify-between mb-8 sm:mb-12">
        <h1 className="font-serif text-2xl font-extrabold text-stone-900 dark:text-white uppercase tracking-tighter">
          Draft
        </h1>
        <div className="flex items-center gap-4">
          ...
          {/* FIX 1 — translate-x-4.5 → translate-x-[18px]; added overflow-hidden + duration-200 */}
          <button
            type="button"
            onClick={() => setPublished(!published)}
            className={`w-10 h-5 rounded-full relative flex items-center px-1 transition-colors overflow-hidden ${
              published ? 'bg-green-600' : 'bg-stone-300 dark:bg-stone-700'
            }`}
          >
            <div
              className={`w-3.5 h-3.5 bg-white rounded-full transition-all duration-200 ${
                published ? 'translate-x-[18px]' : 'translate-x-0'
              }`}
            />
          </button>
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 sm:gap-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2 min-h-12 p-3 bg-white dark:bg-stone-950 rounded-2xl border border-stone-100 dark:border-stone-800">
              {tags.map((t, i) => (
                <span
                  key={i}
                  className="flex items-center gap-1.5 px-3 py-1 bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 rounded-full text-xs font-bold transition-all"
                >
                  {t}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 pt-12">
          ...
          <button
            type="button"
            onClick={() => router.back()}
            className="w-full sm:w-auto px-8 py-4 text-sm font-bold text-stone-400 hover:text-stone-900 dark:hover:text-white transition-colors"
          >
            Cancel
          </button>
        </div>
