...
export default function ShareButtons({ title, slug }: Props) {
  ...
  return (
    <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none min-w-0">
      <span className="text-sm font-medium text-stone-500 dark:text-stone-400 shrink-0 mr-1">
        Share:
      </span>
      ...
      {SHARE_PLATFORMS.map((platform) => (
        <button
          ...
          className={`w-8 h-8 shrink-0 flex items-center justify-center rounded-full border border-stone-200 dark:border-stone-700 text-stone-500 dark:text-stone-400 transition-all duration-200 cursor-pointer ${platform.color}`}
        >
          {platform.icon}
        </button>
      ))}
      ...
      <button
        ...
        className={`w-8 h-8 shrink-0 flex items-center justify-center rounded-full border transition-all duration-200 cursor-pointer ${
          copied
